import math
caption = "Infinite spiral"
fps = f"Frames per second: "

captions = [
    "Video game",
    "Something is hidden in this game, find it!",
    "Close your eyes and play a game! ...or don't close your eyes and play a game!",
    "'Pygame doesen't suck, you do!' The Art of War - Sun Tzu",
    "You can do anything here. (theoretically)",
    "This game has no goal, but if you want, you can find one.",
    "¯\\_(ツ)_/¯",
    "Everything is awesome!",
    ":/",
    "♥♥♥",
    "Triangulate!",
    "Siminmindav Infók és weboldalak (I have my own website :D)",
]

instructions = [
    "                     ---Controls---                     ",
    "Space:                               Makes a new square.",
    "Left mouse button:             Drags the middle square.",
    "Middle mouse button:         Changes the changable variable.",
    "Mouse wheel up:                Increases the selected variable.",
    "Mouse wheel down:            Decreases the selected variable.",
    "Right mouse button:            Deletes the last summoned square.",
    ]

variables = [
    "Speed: ",
    " degree/frame",
    "Size: ",
    " px^2",
    "Radius: ",
    " px",
    "Red: ",
    " 1bit value",
    "Green: ",
    " 1bit value",
    "Blue: ",
    " 1bit value",
    "opacity: ",
    " 1bit value",
    "rect: ",
    " bool",
    "circle: ",
    " bool",
    "line: ",
    " bool",
    "background opacity: ",
    " 1bit value",
    "angle: ",
    " degree",
    "nothing",
]

developer = [
    "Developed by: Siminmin",
    "GitHub link:",
    "https://github.com/Siminmindav/spinner",
    "",
    "Music:",
    "szerő - dal",
    "szerő2 - dal2",
    "",
    "Thank you for playing!",
]

savings = [
    "Selected file:",
    "Name the file",
]

trashing = [
    "Delete all?",
]
leaving = [
    "Leave the game?",
]

textlist = [
    ["Choose window size", (115, 20), 0],
    ["A few sizes:", (13, 70), 0],
    ["800x800", (20, 120), (13, 114, 90, 35)],
    ["700x700", (20, 170), (13, 164, 90, 35)],
    ["fullscreen", (20, 220), (13, 214, 99, 35)],
    ["Type size here:", (220, 70), 0],
    ["X                  px", (220,120), (243, 114, 90, 35)],
    ["", (250,120), 0],
    ["Y                  px", (220,170), (243, 164, 90, 35)],
    ["", (250,170), 0],
]

